...
public class SyncOrCleanUtilCodeQuery extends BaseSyncQuery {

    @Override
    protected void getAllUnSyncRowAndSendOrDelete(Realm realm) {
        RealmResults<UtilCodeTable> realmResults =
                realm.where(UtilCodeTable.class)
        ...
        if(realmResults.size()!=0){
            Stream.of(realmResults).forEach(utilCodeTable -> {
                haveUnSyncData=true;
                SendPacket.sendEncryptionMessage(App.getAppContext(), SocketJsonMaker
                        .utileCodeSocket(SharedPreferenceProvider.getOrganization(App.getAppContext()),
                                SharedPreferenceProvider.getUserId(App.getAppContext()),new UtilCodeTable(utilCodeTable.getId(),utilCodeTable.getParentId(),utilCodeTable.getName(),utilCodeTable.getCode(),utilCodeTable.getParentName(),utilCodeTable.getParentCode(),utilCodeTable.getB5IdRefIds(),utilCodeTable.getSyncState())
                        ), true);
            });
